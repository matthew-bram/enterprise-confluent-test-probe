...
**Key Benefits:**
- 🚀 **Fast Feedback**: Unit tests execute in ~5-10s (vs ~5min for all tests)
- 🎯 **Selective Testing**: Run only the tests you need (unit, component, or both)
- 📊 **Coverage Flexibility**: Generate coverage reports for any test type combination
- 🔧 **Developer Ergonomics**: Simple, consistent commands for humans and AI
- ⚙️ **Always Compile Dependencies**: `-am` flag ensures dependencies build automatically
...
- `test-probe-common` → Foundation models and contracts
- `test-probe-core` → Actor system, FSM, Kafka streaming
- `test-probe-interfaces` → REST/CLI/gRPC interfaces

**Dependency Chain**: `interfaces` → `core` → `common`
...
│                                                                  │
│  Properties:                                                     │
│    - skipUnitTests: false (default)                             │
│    - skipComponentTests: false (default)                        │
│                                                                  │
│  Profiles:                                                       │
│    ┌────────────────┬──────────────┬───────────────────┐       │
│    │  unit-only     │ component-   │ coverage-         │       │
│    │                │ only         │ aggregate         │       │
│    │  skipComponent │ skipUnit     │ aggregateOnly:    │       │
│    │  Tests=true    │ Tests=true   │ true              │       │
│    └────────────────┴──────────────┴───────────────────┘       │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                  Module POMs (core, interfaces, common)          │
│                                                                  │
│  ScalaTest Plugin:                                              │
...
│                       Test Execution                             │
│                                                                  │
│  Unit Tests (ScalaTest):      Component Tests (Cucumber):       │
│    - *Spec.scala files         - *TestRunner.class             │
│    - Fast (~5-10s)             - Slow (~4-5min, Docker)         │
│    - No Docker required         - Requires Testcontainers       │
└─────────────────────────────────────────────────────────────────┘
```
...
# ✅ CORRECT - Automatically builds dependencies
mvn test -pl test-probe-interfaces -am
```
...
```xml
<!-- ❌ WRONG - Parameter not supported -->
<configuration>
    <skip>${skipUnitTests}</skip>
</configuration>

<!-- ✅ CORRECT - Uses supported parameter -->
<configuration>
    <skipTests>${skipUnitTests}</skipTests>
</configuration>
```
...
|------------|-------------|-------|
| `mvn test -pl test-probe-core` | `mvn test -pl test-probe-core -am` | Add `-am` for dependencies |
| `mvn test -Punit-only` | No change | Works project-wide |
| `mvn test -Pcomponent-only` | No change | Now actually works! |
| `mvn compile -pl test-probe-core` | `mvn compile -pl test-probe-core -am` | Add `-am` for dependencies |

### For CI/CD Pipelines

**Important**: All tests (unit and component) require Docker/Testcontainers because even
ScalaTest specs may use `TestcontainersManager` for Schema Registry integration. Tests
must be run locally where Docker is available.

```yaml
# GitHub Actions workflow - Compile only (no Docker available)
jobs:
  build:
...
**Local Testing**: Run tests locally with Docker Desktop:
```bash
# All tests (requires Docker)
mvn test

# Unit tests only (still requires Docker for some specs)
mvn test -Punit-only

# Component tests only (Cucumber BDD)
mvn test -Pcomponent-only
```
...
**Symptom**: ScalaTest tests execute when running `mvn test -Pcomponent-only`
...
**Symptom**: `mvn test -pl test-probe-interfaces` fails with "class not found"
...
```bash
mvn clean test scoverage:report -Punit-only -pl test-probe-core -am 2>&1 | grep "Tests run:"
```
