#!/bin/bash
# test-component.sh - Run component tests with automatic environment management
#
# Usage:
#   ./scripts/test-component.sh [-m <module>] [-h|--help]
#
# Examples:
#   ./scripts/test-component.sh -m core
#   ./scripts/test-component.sh -m interfaces
#   SKIP_K8S_SCALING=1 ./scripts/test-component.sh -m core  # Skip K8s (CI mode)
...
show_help() {
  cat << EOF
${COLOR_BOLD}${COLOR_CYAN}test-component.sh${COLOR_RESET} - Run component tests with environment management
...
${COLOR_BOLD}EXAMPLES:${COLOR_RESET}
  ...
  ./scripts/test-component.sh -m services
...
${COLOR_BOLD}FEATURES:${COLOR_RESET}
  ✓ Runs @ComponentTest tagged scenarios only
...
main() {
  ...
  log_header "Component Tests: $MODULE_NAME"
  ...
  log_step_timed 4 5 "Running component tests for $MODULE_NAME"
  ...
  log_progress "Running component tests (this may take 2-4 minutes)"
  ...
  export CUCUMBER_FILTER_TAGS="@ComponentTest and not @Ignore"

  if ./mvnw "${mvn_args[@]}" 2>&1 | tee /tmp/component-test-output.log; then
    TEST_EXIT_CODE=0
  else
    TEST_EXIT_CODE=$?
  fi
  ...
  if [[ $TEST_EXIT_CODE -eq 0 ]]; then
    # Parse test summary
    local test_count=$(grep "Tests run:" /tmp/component-test-output.log | tail -1 | grep -o "Tests run: [0-9]*" | grep -o "[0-9]*" || echo "unknown")
    local elapsed=$(get_elapsed_time)

    log_success "Component tests passed for $MODULE_NAME! ✓"
    ...
    log_error "Component tests failed for $MODULE_NAME (exit code: $TEST_EXIT_CODE)"
    ...
    if grep -q "Tests run:" /tmp/component-test-output.log; then
      grep "Tests run:" /tmp/component-test-output.log | tail -1 | sed 's/^/  /'
    fi

    if grep -E "<<< FAILURE|<<< ERROR" /tmp/component-test-output.log > /dev/null; then
      log_error "Failed tests:"
      grep -E "<<< FAILURE|<<< ERROR" /tmp/component-test-output.log | sed 's/^/  - /' | head -10

      local failure_count=$(grep -E "<<< FAILURE|<<< ERROR" /tmp/component-test-output.log | wc -l | tr -d ' ')
    ...
    echo ""
    log_info "For full details, see: /tmp/component-test-output.log"
    log_info "Run with -X for stack traces:"
    log_info "  ./mvnw test -X -Pcomponent-only -pl $MODULE_NAME '-Dcucumber.filter.tags=@ComponentTest and not @Ignore'"
